...
namespace SvgPathProperties.UnitTests
{
    public class GetPartsTests
    {
        ...
        [Fact]
        public void TestingThGetPartsWithSimplePath_Fluent()
        {
            var properties = SvgPathUtils.FluentFromStr("m10,0l10,0");
            var parts = properties.GetParts();

            Assert.Single(parts);

            properties = SvgPathUtils.FluentFromStr("m10,0l10,0l10,0");
            parts = properties.GetParts();
        ...
        [Fact]
        public void TestingTheGetPartsWithSimplePath_Fluent()
        {
            var properties = SvgPathUtils.FluentFromStr("M100,200 C100,100 250,100 250,200 S400,300 400,200");
            var parts = properties.GetParts();
        ...
        [Fact]
        public void Issue15_Fluent()
        {
            var def = "M0,0 c 0.025,-0.052 0.081,-0.1387 0.2031,-0.2598 0,0 0,0 0,0";
            var properties = SvgPathUtils.FluentFromStr(def);
            properties.GetParts(); //The above path used to hang the programd

            def = "M0,0 c 0.025,-0.052 0.081,-0.1387 0.2031,-0.2598 0,0 0,0 0,0 c 0.1865,-0.31055 0.3632,-0.71289 0.5371,-1.22266 0.1963,-0.40625 0.3261,-0.78516 0.3857,-1.13184 0,-0.008 0,-0.0156 0,-0.0225";
            properties = SvgPathUtils.FluentFromStr(def);
            properties.GetParts();
        }
    }
}
